...
\begin{definition}{映射/函数}
  设$A,B$是两个非空集合，若存在一个对应法则$f$，使得对$A$中的每个元素$x$，在$B$中都有\textcolor{orange}{唯一确定}的元素$y$按法则$f$与之对应，则称$f$为从$A$到$B$的\underline{映射}或从$A$到$B$的\underline{函数}，记作$$f:A\rightarrow B,$$其中$y$称为元素$x$（在映射$f$下）的\underline{像}，或者称为$x$对应的\underline{函数值}，记作$f(x)$，即$$y=f(x),$$元素$x$称为元素$y$（在映射$f$下）的\textcolor{orange}{一个}\underline{原像}；集合$A$称为映射$f$的\underline{定义域}，$A$中所有元素的像组成的集合称为映射$f$的\underline{值域}。

  若我们知道$f(x)$的表达式（如\ref{MapExample2}），则使得$f(x)$有意义的所有$x$组成的集合称为\underline{自然定义域}。
\end{definition}

\begin{remark}
  考虑到一元函数（只有一个变量的函数，我们接下来就要讨论这个）和多元函数（有多个变量的函数，本读本可能不讲）的统一，我们采用了这种广义的定义方式。这种定义和Walter Rudin所著的《数学分析原理》相同，但和国内现行的许多教材可能不尽相同。这些教材一般在定义函数时限定定义域和值域为$\mathbb{R}$的子集，也就是定义了一元函数。\textcolor{red}{接下来，为了叙述的便利，在没有特殊说明的情况下，我们的“函数”也指的是一元函数，如初中所学的一次函数$y=2x$.}
\end{remark}
...
\begin{example}\label{MapExample2}
  让每一个$x\in\mathbb{R}$对应$2x$，就可以给出一个从$\mathbb{R}$到$\mathbb{R}$的映射$$f:\mathbb{R}\rightarrow\mathbb{R}.$$此时我们就可以写成：$$f(x)=2x(x\in\mathbb{R}).$$这里的$(x\in\mathbb{R})$由于是自然定义域，通常可以省去不写，直接写$f(x)=2x$.
\end{example}
...
给出解法之前先说明一下，函数的记号不一定要用$f(x)$，也可以用其他字符表示不同的对应关系，比如说$g(x),\phi (x),Loss(x)$，如果自变量的记号也是其它字符，比如$y$，那么就可以写成类似$g(y)$的形式。不同的记号用来表示不同的函数，可以有效避免混淆。
...
\textbf{定义域、对应法则、值域}是函数的三要素。

\subsection{单射、满射与一一映射}

\begin{definition}{映射的相等}
  设$f:A\rightarrow B$，且$g:A\rightarrow B$，若$\forall x\in A,f(x)=g(x)$，则称映射（函数）$f$和$g$是\underline{相等}的。记作$f=g$.
\end{definition}

通俗地解释就是三要素相同。

\begin{example}
  函数$y=x$和$y=\sqrt{x^2}$相等，和$u=\sqrt[3]{x^3}$相等，但和$y=\dfrac{x^2}{x}$不相等。
\end{example}
