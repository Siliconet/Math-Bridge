...
\subsection{区间}
当我们用不等号表示数的范围时，我们一般不会觉得繁琐。但是，当这个范围分成几段时，就容易显得麻烦。比方说，不等式$x^4-19x^3+110x^2-197x+69>0$（这个不等式的解法我们目前还没学，但是不影响接下来的学习）的解集为$x<\frac{1}{2} \left(7-\sqrt{37}\right)\text{或}6-\sqrt{13}<x<\frac{1}{2} \left(\sqrt{37}+7\right)\text{或}x>\sqrt{13}+6$，有点麻烦。
...
那么，上面的不等式的解集就可以表示为$(-\infty,\frac{1}{2} \left(7-\sqrt{37}\right))\cup (6-\sqrt{13},\frac{1}{2} \left(\sqrt{37}+7\right))\cup (\sqrt{13}+6,+\infty)$，虽然也比较长，但是相比之前的还是要简短一点。
...
\begin{proof}
  ...
  原方程可化为$\left(x+\frac{b}{2a}\right)^{2}=\frac{b^{2}-4ac}{4a^{2}}$，

  当$b^2-4ac=0$时，上式右边等于0，则原方程有两个相等的实数根$x_1=x_2=-\frac{b}{2a}$.

  再证必要性，
  
  若关于$x$的实系数一元二次方程$ax^2+bx+c=0$有两个相等的实数根，则由一元二次方程根与系数的关系可得$x_{1}+x_{2}=2x_{1}=-\frac{b}{a},x_{1}x_{2}=x_{1}^{2}=\frac{c}{a},$

  因此$\left(-\frac{b}{2a}\right)^{2}=\frac{c}{a}$，即$b^2-4ac=0$.
\end{proof}
...
逻辑学上，我们用“或”联结两个命题，得到的新命题记作“$p\lor q$，读作“$p$或$q$”。
...
将条件和结论互换，就得到原命题的\underline{逆命题}：

\begin{equation}\label{NiMingTi}
  \text{如果}q\text{，则}p.
\end{equation}

分别否定条件和结论，就得到原命题的\underline{否命题}：

\begin{equation}\label{FouMingTi}
  \text{如果}\neg p\text{，则}\neg q.
\end{equation}

将条件和结论互换并分别否定，就得到原命题的\underline{逆否命题}：
...
命题的形式是相对的。如果我们将命题\ref{NiMingTi}看作原命题，那么命题\ref{YuanMingTi}就是它的逆命题。也就是说，命题\ref{YuanMingTi}和\ref{NiMingTi}\textbf{互为}逆命题。我们称这两个命题为\underline{互逆命题}。命题\ref{FouMingTi}和\ref{NiFouMingTi}也是\underline{互逆命题}。

同样地，命题\ref{YuanMingTi}和\ref{FouMingTi}是\underline{互否命题}，命题\ref{NiMingTi}和\ref{NiFouMingTi}也是\underline{互否命题}。

命题\ref{YuanMingTi}和\ref{NiFouMingTi}\underline{互为逆否命题}，命题\ref{FouMingTi}和\ref{NiMingTi}也\underline{互为逆否命题}。
...
\begin{exercise}\label{202407081031}
  $\text{设}a,b\in\mathbb{R},\text{集合}\{1,a+b,a\}=\left\{0,\frac{b}{a},b\right\},\text{则}b-a=$\underline{\hbox to 15mm{}}.
\end{exercise}
