...
特别的是，如果一个命题中含有全称量词或者存在量词，在写它的否定时需要做出一些改变。

先来看两个例子：

命题“所有的数都是有理数”的否定是“不是所有的数都是有理数”，换一个说法，就是“存在不是有理数的数”。这里的\textcolor{red}{全称量词}在否定后变为了\textcolor{blue}{存在量词}。

命题“存在无实数解的一元二次方程”的否命题是“所有一元二次方程都有实数解”。这里的\textcolor{blue}{存在量词}在否定后变为了\textcolor{red}{全称量词}。

把上面的内容一般化。“$\exists x\in M, p(x)$”的否定是“$\forall x{\in}M,\neg p(x)$”，“$\forall x\in M, q(x)$”的否定是“$\exists x{\in}M, \neg q(x)$”.

\begin{example}
  命题“$\forall x>1,x+2>3$”的否定是“$\exists x>1,x+2\leq 3$”.
\end{example}

\begin{example}
  命题“$\exists x\in\mathbb{R},x^2-1\geq 0$”的否定是“$\forall x\in\mathbb{R},x^2-1<0$”.
\end{example}
